...
def parse_uniswap_trade(item, w3):
    ...
    if item['input'].startswith('0xc04b8d59'):
        return None # uniswap v3 router: exactInput
    ...
    if (item['input'].startswith('0x7ff36ab5')          # uniswap v2 router: swapExactETHForTokens
            ...
            or item['input'].startswith('0xb6f9de95')   #TODO missing swapExactETHForTokensSupportingFeeOnTransferTokens
            ):
        ...
        for log in logs:
            if (Web3.toChecksumAddress(log['address']) == WETH_ADDR 
                    and log['topics'][0].hex() == '0xe1fffcc4923d04b559f4d29a8bfc6cda04eb5b0d3c460751c2402c5c5cc9109c'):
    ...
    for tlog in logs:
        if (len(tlog['topics']) >= 3 and tlog['topics'][0].hex() == '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef'):
    ...
    def export_item(self, item, fields):
        if item['input'].startswith('0xa9059cbb'): # erc20:transfer
            filtered_item = parse_erc20_transfer(item, self.w3)
                ...
                or item['input'].startswith('0xc04b8d59') or item['input'].startswith('0x5c11d795')
                or item['input'].startswith('0xb6f9de95')):
